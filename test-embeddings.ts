...
function cosineSimilarity(embedding1: number[], embedding2: number[]): number {
  ...
  function generateNaturalDescriptions(metadata: ProductMetadata): string[] {
    ...
    if (metadata.colors.length > 0) {
      const colorDesc = metadata.colors.length === 1
    ...
    if (metadata.materials.length > 0) {
      const materialDesc = metadata.materials.length === 1
    ...
    const testProducts: ProductMetadata[] = [
      {
        ...
        visualDescription: "Kulmasähköpöytä, jossa on moderni ja minimalistinen muotoilu. Pöydän runko on metallinen ja väriltään harmaa, kun taas pöytälevy on valkoinen. Sähkösäätöinen mekanismi mahdollistaa pöydän korkeuden säätämisen ergonomisesti sopivaksi eri käyttäjille.",
      },
      {
        ...
        visualDescription: "Ergonominen työtuoli mustalla verhoilulla ja puisilla yksityiskohdilla. Tuolissa on säädettävä korkeus, käsinojat ja tukeva selkänoja.",
      }
    ];
    ...
    try {
      ...
      for (const query of searchQueries) {
        ...
        similarities.forEach((result, index) => {
          console.log(`\nTulos ${index + 1}:`);
          console.log(`Tuote: ${result.product.category} (${result.product.colors.join(", ")})`);
